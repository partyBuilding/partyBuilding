...
public interface MeetingManagementMapper {
    //根据可选条件查询会议列表
    List<MeetingManagement> getMeetingManagement(@Param("title") String title,@Param("typeId") int typeId,@Param("auditId") int auditId,@Param("id") int id,@Param("meetingOrCourse") int meetingOrCourse,@Param("status") int status);
    //新增会议
